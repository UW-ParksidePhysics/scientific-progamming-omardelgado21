...
earth_center = vp.vector(40., 40., 0.)
earth_dimensions = vp.vector(40., 40., 0.)
earth = vp.box(pos=earth_center, size=earth_dimensions, color=vp.color.green)

Earth = vp.text(text="EARTH", pos=vp.vector(50, 50, -20), align='center', height=15, color=vp.color.blue, axis=vp.vector(1, 1, 0), up=vp.vector(0, 0, 1))

mars_center = vp.vector(0., 0., 0.)
mars_dimensions = vp.vector(40., 40., 0.)
mars = vp.box(pos=mars_center, size=mars_dimensions, color=vp.color.red)

Mars = vp.text(text="MARS", pos=vp.vector(-0, -0, -20), align='center', height=15, color=vp.color.red, axis=vp.vector(1, 1, 0), up=vp.vector(0, 0, 1))

moon_center = vp.vector(-40., -40., 0.)
moon_dimensions = vp.vector(40., 40., 0.)
moon = vp.box(pos=moon_center, size=moon_dimensions, color=vp.color.gray(.8))

Moon = vp.text(text="MOON", pos=vp.vector(-50, -50, -20), align='center', height=15, color=vp.color.gray(.8), axis=vp.vector(1, 1, 0), up=vp.vector(0, 0, 1))

initial_position_earth = vp.vector(20., 20., 0.1)
initial_velocity_earth = vp.vector(10., 10., 10.)
grav_earth = -9.8
ball_earth = vp.sphere(pos=initial_position_earth, radius=1., color=vp.color.blue, make_trail=True)

initial_position_mars = vp.vector(-20., -20., 0.1)
initial_velocity_mars = vp.vector(10., 10., 10.)
grav_mars = -3.7
ball_mars = vp.sphere(pos=initial_position_mars, radius=1., color=vp.color.yellow, make_trail=True)

initial_position_moon = vp.vector(-60., -60., 0.1)
initial_velocity_moon = vp.vector(10., 10., 10.)
grav_moon = -1.6
ball_moon = vp.sphere(pos=initial_position_moon, radius=1., color=vp.color.white, make_trail=True)
